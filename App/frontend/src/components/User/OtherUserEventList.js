...
import { allSearchedUserEvents } from '../../api/handleEvent';

const OtherUserEventList = ({ navigation, searchedUser }) => {
  ...
  const fetchData = async (clearAll) => {
    try {
      setLoadingMore(true);
      const res = await allSearchedUserEvents(token, page, searchedUser)
...
export default OtherUserEventList;
